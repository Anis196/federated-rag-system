...
export function AnimatedAIChat() {
    ...
    const commandSuggestions: CommandSuggestion[] = [
        { 
            ...
            label: "Recommend", 
            description: "Recommend the items for user", 
            prefix: "/recommend" 
        },
        { 
    ...
    const handleSendMessage = async () => {
        if (value.trim()) {
            ...
            await sendMessage(messageText, messageAttachments.length > 0 ? messageAttachments : undefined);
        }
    };
            ...
            <div className="w-full max-w-2xl mx-auto relative">
                ...
                >
                    ...
                    >
                        ...
                        <div className="p-4">
                            {/* Messages area */}
                            <div className="messages max-h-64 overflow-auto mb-4 space-y-3">
                                {messages.length === 0 && (
                                    <div className="text-sm text-white/40">Start the conversation — ask a question or use a command.</div>
                                )}
                                {messages.map((m) => (
                                    <div key={m.id} className={m.role === 'user' ? 'flex justify-end' : 'flex justify-start'}>
                                        <div className={
                                            `px-4 py-2 rounded-lg max-w-[75%] text-sm leading-snug ${m.role === 'user' ? 'bg-white text-[#0A0A0B]' : 'bg-white/5 text-white'}`
                                        }>
                                            {m.content}
                                            <div className="text-[10px] text-white/40 mt-1 text-right">{new Date(m.timestamp).toLocaleTimeString()}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
